...
public class BinTree {
  ...
  public static BinTree merge(BinTree... trees) {
    ...
    if(trees.length == 2) {
      ...
      BinTree tLeft = new BinTree();
      tLeft.root = trees[1].root.getLeft();
      BinTree tRight = new BinTree();
      tRight.root = trees[1].root.getRight();

      trees[0].insert(trees[1].root.getValue());
      trees[0] = merge(trees[0], tLeft, tRight);
      return trees[0];
    }
    return merge(merge(trees[0], trees[1]), merge(Arrays.copyOfRange(trees, 2, trees.length)));
